...
@SQLDelete(sql = "UPDATE ORG_DET_ORG_UNIT SET IS_DELETED = 1 WHERE ODO_PK_ID = ?")
@Where(clause = "IS_DELETED = 0 OR IS_DELETED IS NULL")
public class OrganizationUnit extends BaseTenantEntity
{
    private static final long serialVersionUID = 759004381838816377L;
    @Id
    ...
    @Column(name = "ODO_AT_DESCRIPTION", length = 1024)
    private String description;
    @Column(name = "ODO_AT_MISSION", length = 1024)
    private String mission;
    @Column(name = "ODO_AT_VISION", length = 1024)
    ...
    @JoinTable(name = "ORG_MAP_DIMENSION", joinColumns = { @JoinColumn(name = "ODO_FK_ID", nullable = false, updatable = false) }, inverseJoinColumns = { @JoinColumn(name = "OMS_FK_ID", nullable = false, updatable = false) })
    private List<OrganizationUnitType> orgTypeMappings;
    ...
    public OrganizationUnit() {
        ...
        this.orgTypeMappings = new ArrayList<OrganizationUnitType>();
    ...
    public List<OrganizationFieldData> getFiledData() {
        return Collections.unmodifiableList((List<? extends OrganizationFieldData>)this.filedData);
    }
    
    public void setFiledData(final List<OrganizationFieldData> filedData) {
        this.filedData.clear();
        if (filedData != null && !filedData.isEmpty()) {
            for (final OrganizationFieldData iterator : filedData) {
                this.filedData.add(iterator);
            }
        }
    }
    
    public List<OrganizationUnitType> getOrgTypeMappings() {
        return this.orgTypeMappings;
    }
    
    public void setOrgTypeMappings(final List<OrganizationUnitType> orgTypeMappings) {
